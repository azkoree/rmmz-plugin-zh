...
/*:zh
 * @target MZ
 * @plugindesc v1.241 战斗动画的自动化及超强化。
 ...
 * 
 * 通过arcRandonALL模板的功能，会变成向目标全体投射动画的演出效果。
 * 并且可以设置更加详细的参数。
 * 
 * <D-Animation:arcRandomAll> // 向全体投射
 * id = 82 // 水/单体2（省略时为技能设置）
 * repeat = 20 // 重复次数
 * interval = 1 // 间隔
 * arrival = 7 // 到达所需帧数
 * arcY = -200 // 抛物线高度
 ...
 * @type number
 * @default 4
 * @desc 用于interval和wait等指定的计算速率。
 * 初始值为4。即以4/60秒为单位进行处理。
 * 
 ...
 * @type boolean
 * @default true
 * @desc 计算动画位置时考虑对象的放大率。
 * 此项目也适用于DynamicMotion。
 * 
 * @param overWindow
 * @text 显示在窗口之上
 * @parent <Animation Position>
 ...
 * @default 0
 * @desc 为了冲突对策而更改处理。
 * 如果是1的话，YEP_BattleEngineCore也许也能运行……。
 * 
 ...
 * @option 2:足元 @value 2
 * @option 3:画面 @value 3
 * @desc 动画的位置。0:头上, 1:中心, 2:足下, 3:画面。
 * 画面以外会按对象人数显示。
 * 
 * @param interval
 * @text 间隔
 ...
 * @parent <Real Time>
 * @type string
 * @desc 旋转率。Math.PI * 2旋转一圈。
 * 与MV的行为相同。与rotationZ方向相反。
 * 
 * @param rotationX
 * @text 旋转率X
 * @parent <Real Time>
 * @type string
 * @desc X方向的旋转率。Math.PI * 2旋转一圈。
 * 
 * @param rotationY
 * @text 旋转率Y
 * @parent <Real Time>
 * @type string
 * @desc Y方向的旋转率。Math.PI * 2旋转一圈。
 * 
 * @param rotationZ
 * @text 旋转率Z
 * @parent <Real Time>
 * @type string
 * @desc Z方向的旋转率。Math.PI * 2旋转一圈。
 * 
 * @param opacity
 * @text [MV]不透明度
 * @parent <Real Time>
 * @type string
 * @desc 不透明度。255为不透明，0为透明。
 * 此项目仅限MV动画使用。
 * 
 * @param color
 * @text [MV]色调
 * @parent <Real Time>
 * @type string
 * @desc 更改色调。例：[255, 255, 255, 255]
 * 顺序为红、绿、蓝、强度。此项目仅限MV动画使用。
 * 
 * @param cellStartX
 * @text [MV]单元格开始X坐标
 * @parent <Real Time>

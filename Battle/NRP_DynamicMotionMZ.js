...
/*:zh
 * @target MZ
 * @plugindesc v1.261 技能执行时，自由调用动作。
 ...
 * @help 
 * 从技能（道具）中自动调用动作。
 * 本插件的运行需要NRP_DynamicAnimationMZ.js。
 * 
 * ■使用方法
 * 例如，在技能备注栏输入如下内容。
 * 
 * <D-Setting:NoStep> // 禁止自动前进
 * <D-Motion:near> // 接近目标
 * frame = 10 // 接近用时
 * </D-Motion>
 * <D-Motion:attack/> // 挥动武器
 * <D-Animation/> // 动画
 * <D-Motion:return/> // 返回位置
 * 
 * ス这样设置后，角色会接近目标进行攻击。
 * 
 * <D-Motion:near&jump&roll> // 接近、跳跃、旋转
 * frame = 20 // 接近用时
 * arcY = -200 // 跳跃高度
 * rotation *= 5 // 旋转次数
 * </D-Motion>
 ...
 * ■使用条款
 * 没有特别的限制。
 * 可以自由改変、再配布、商用、权利表示也是任意的。
 * 作者不承担责任，但对于不完善之处会在可能的范围内对应。
 * 
 * @param templateList
 * @text 模板一览
 ...
 * @type string
 * @default ds
 * @desc 可以用指定字符串省略设置标签名。
 * 例：<D-Setting:NoStep> -> <ds:NoStep>
 * 
 * @param setStartMotion
 * @text 开始动作的设置
 * @type select
 * @option 0:始终有 @value 0
 * @option 1:始终无 @value 1
 * @option 2:仅指定动作时无 @value 2
 * @default 2
 * @desc 设置攻击或技能的开始动作的有无。
 * 0:始终有, 1:始终无, 2:仅指定动作时无
 * 
 ...
 * @text 名称
 * @type string
 * @desc 用于识别的备注。请取一个容易理解的名字。
 * 实际上在技能・物品的备注栏中指定这个也可以使用。
 * 
 * @param delay
 * @text 延迟（迟延）
 * @type string
 ...
 * @text 图像
 * @type string
 * @desc 将战斗者或角色的图像更改为指定文件。
 * 切换画面后恢复原状。
 * 
 * @param imageIndex
 * @text 图像索引
 * @type number
 * @desc 临时更改角色图像的索引(0-7)。
 * 仅限地图。切换画面后恢复原状。
 * 
 * @param <Repeat>
 * @text ＜重复＞
 ...
 * @option 2:足下 @value 2
 * @option 3:画面 @value 3
 * @desc 动作的目标位置。0:头上, 1:中央, 2:足下, 3:画面。
 * 
 * @param interval
 * @text 间隔
 * @parent <Repeat>
 ...
 * @parent <Repeat>
 * @type string
 * @desc 当技能对象为多个时执行动作的时间差。
 * 如果不将『每个对象重复』设为ON则无意义。
 * 
 * @param performerDelay
 * @text 每个动作对象的时间差
 * @parent <Repeat>
 * @type string
 * @desc 当动作对象为多个时执行动作的时间差。
 * 
 * @param mirror
 * @text 左右反转
 * @parent <Repeat>
 ...
 * @parent <Motion>
 * @type select
 * @option 攻击（attack） @value attack
 * @option 前进（walk） @value walk
 * @option 通常待机（wait） @value wait
 * @option 詠唱待机（chant） @value chant
 * @option 防御（guard） @value guard
 * @option 伤害（damage） @value damage
 * @option 回避（evade） @value evade
 * @option 突刺（thrust） @value thrust
 * @option 挥动（swing） @value swing
 * @option 飞行道具（missile） @value missile
 * @option 通用技能（skill） @value skill
 * @option 魔法（spell） @value spell
 * @option 物品（item） @value item
 * @option 逃跑（escape） @value escape
 * @option 胜利（victory） @value victory
 * @option 瀕死（dying） @value dying
 * @option 状态异常（abnormal） @value abnormal
 * @option 睡眠（sleep） @value sleep
 * @option 战斗不能（dead） @value dead
 * @desc 执行的动作名。
 * 例：walk, wait等。另外"attack"会挥动武器。
 * 
 ...
 * @option 6:右 @value 6
 * @option 8:上 @value 8
 * @option 面向对象 @value a.turnTowardDirection(b)
 * @option 对象的反方向 @value a.turnAwayDirection(b)
 * @desc 指定角色的方向。
 * 除了数值，也可以用up, down, left, right指定。
 * 
 * @param pattern
 * @text 模式（实时）
 * @parent <Map Only>

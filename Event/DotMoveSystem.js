...
/*:
@target MV MZ
@plugindesc 像素级移动系统 v2.2.4
@author unagi ootoro
@url https://raw.githubusercontent.com/unagiootoro/RPGMZ/master/DotMoveSystem.js
@help
这是一个可以按像素（dot）为单位进行移动的插件。

【使用方法】
基本上只需导入即可使用，但通过设置以下内容可以实现更精细的控制。
...
这样写，就可以按像素为单位朝指定的角度方向移动。

■ 以像素为单位让事件朝玩家的方向移动
在移动路线的脚本中
this.dotMoveToPlayer();
这样写，就可以按像素为单位让事件朝玩家的方向移动。

■ 移动到指定的坐标
在移动路线的脚本中
this.moveToTarget(X坐标, Y坐标);
这样写，就可以让它朝指定的坐标移动。
※1 如果途中撞到墙壁等物，到达坐标会产生偏差。
※2 针对指定坐标的移动，以角色的左上角为原点进行。

■ 事件接触判定的设置
在事件第1页EV页面的最开头的事件指令中写入注释，
并在注释中记载以下内容，即可更精细地设置事件接触判定。
・横轴的接触范围
<widthArea: 接触宽度(0～1之间的小数)>

・纵轴的接触范围
<heightArea: 接触宽度(0～1之间的小数)>

如果将事件优先级设置为「与普通角色相同以下」或「与普通角色相同以上」，
则横轴的接触范围与纵轴的接触范围都会被用作接触范围。
当设置为「与普通角色相同」时，向上或向下接触会使用横轴的接触范围，
而向左或向右接触时会使用纵轴的接触范围。

如果横轴与纵轴都没有设置，则适用 0.5。

■ 事件大小的设置
在事件第1页EV页面的最开头的事件指令中写入注释，
并在注释中记载以下内容，即可更精细地设置事件的大小。
・横向大小
<width: 宽度(0.5以上的实数)>
...
例如，想以 width: 2、height: 2 来设置 96 * 96 大小的角色时，写法如下。
显示大于 48 * 48 的角色时，显示起始位置与实际 XY 坐标不同，
因此需要调整显示偏移。
<width: 2>
<height: 2>
<offsetX: 0.5>
...
■ 事件滑动长度的设置
当角色撞到墙壁的拐角时，会朝没有拐角的一侧移动。
如果角色大小小于 1，则滑动宽度取大小的一半；如果为 1 以上，则取 0.5。

另外，在事件第1页EV页面的最开头的事件指令中写入注释后，
并在注释中记载以下内容，即可更精细地设置事件的滑动宽度。
・X轴滑动长度
<slideLengthX: 宽度(0以上的实数)>

・Y轴滑动长度
<slideLengthY: 纵向宽度(0以上的实数)>

■ 从脚本中更改角色大小/显示偏移/滑动宽度
在移动路线的脚本中记载以下脚本，
即可更改角色大小/显示偏移/滑动宽度。
...
・X坐标显示偏移的更改
this.setOffsetX(偏移量的值);

・Y坐标显示偏移的更改
this.setOffsetY(偏移量的值);

・X轴方向滑动宽度的更改
this.setSlideLengthX(滑动宽度的值);

・Y轴方向滑动宽度的更改
this.setSlideLengthY(滑动宽度的值);

・横轴接触范围的更改
this.setWidthArea(接触宽度的值);

・纵轴接触范围的更改
this.setHeightArea(接触宽度的值);
...
获取相对于目标角色的角度。角度计算的原点使用中心点。

Game_CharacterBase#moveByDirection(direction)
以 setMoveUnit 指定的移动单位，朝角色的 direction 方向移动。

Game_CharacterBase#stopMove()
停止角色的移动。如果在停止状态下移动角色，会自动恢复移动。

Game_CharacterBase#resumeMove()
恢复角色的移动。

Game_CharacterBase#cancelMove()
取消由 moveToTarget 等进行的、角色向特定地点的移动。

Game_CharacterBase#checkCharacter(character)
检查是否与参数指定的 character 发生冲突，若发生冲突则返回 CollisionResult 对象。

Game_CharacterBase#checkHitCharacters(targetCharacterClass)
检查是否与 targetCharacterClass 指定的角色类
所对应的所有角色发生冲突，并返回 CollisionResult 对象的数组。
...
(function (DotMoveSystem) {
    ...
    class CharacterCollisionChecker {
        ...
        initialize(character, opt = {}) {
            ...
            // overComplementMode 为 true 时，若冲突宽度超过角色宽度则对冲突宽度进行修正
            this._overComplementMode = opt.overComplementMode == null ? false : opt.overComplementMode;
            // throughIfCollided 为 true 时，若已经发生过冲突则将冲突判定设为无效
        ...
        // 本方法会为了提速而直接修改参数 subjectRect 与 targetRect
        checkCollidedRect(d, subjectRect, targetRect, targetObject) {
            ...
            // throughIfCollided 有效时，若已经处于冲突状态则排除在冲突判定对象之外。
    ...
    class CharacterDotMoveProcess {
        ...
        // 进行冲突判定，从冲突矩形中取得冲突长度，并回退相应的距离
        // 存在多个冲突矩形时，按冲突距离最长的那一份回退距离
        ...
        // 冲突距离大于等于角色的移动距离时，滑动移动距离的量
        // 冲突距离小于角色的移动距离时，滑动冲突距离的量
    ...
    // 使用 CharacterController 更新角色的坐标，
    // 并据此更新角色的各种状态。
    ...
    // 让 CharacterMover 的数据持有存档数据中需要保留的部分
    class MoverData {
    ...
    // 版本ID相同时不会调用 Game_Map#setup，因此地图切换时的初始化处理在此处执行
    const _Scene_Map_start = Scene_Map.prototype.start;
    Scene_Map.prototype.start = function () {
        _Scene_Map_start.call(this);
        // 地图切换时初始化所有角色的临时数据（防止内存泄漏）
        $gameTemp.initCharacterTempDatas();
        // 地图切换时清除地图角色的缓存
        $gameMap.initMapCharactersCache();
        // 地图切换时，将已经与玩家发生冲突的事件排除在启动对象之外
    ...
    Game_CharacterBase.prototype.setPositionPoint = function (point) {
        ...
        // 在循环地图中执行 setPosition 时整数坐标有时会超出范围，此处理用于防止该情况
        if ($gameMap.isLoopHorizontal())
    ...
    // 获取触摸宽度（与角色进行接触判定所需的宽度）。
    // 触摸宽度用于角色的滑动以及事件启动用的冲突判定。
    ...
    Game_CharacterBase.prototype.setPosition = function (x, y) {
        // 首次设置坐标时 $gameMap 与 $dataMap 尚未生成，因此不进行坐标取整
        if ($gameMap && $dataMap) {
            x = $gameMap.roundX(x);
            y = $gameMap.roundY(y);
        }
    ...
    Game_Player.prototype.initMembers = function () {
        ...
        this._shipOrBoatTowardingLand = false; // 管理是否正从船向陆地移动的标志
        this._getOffVehicleIntPos = false; // 管理从载具下来时是否在整数坐标着地的标志
    ...
    Game_Player.prototype.forceMoveOffAirship = function () {
        ...
        // 在向整数坐标着地期间固定飞艇与玩家的朝向
        // 解除固定化在 updateVehicleGetOff 中执行
    ...
    Game_Player.prototype.update = function (sceneActive) {
        ...
        // 将 wasMoving 的获取时机更改为 moveByInput 之后
    ...
    Game_Player.prototype.updateRemoveCollideTriggerEventIds = function () {
        if (this.isMoving() || this.isJumping())
            return; // 从船下来时或跳跃目标的事件，排除在启动对象之外
    ...
    Game_Follower.prototype.update = function () {
        Game_Character.prototype.update.call(this);
        // 跟随者的移动速度由 chaseCharacter 设置，因此此处不设置
    ...
    Game_Follower.prototype.chaseCharacter = function (character) {
        ...
        if (far >= 1) {
            ...
            if (far >= 5) {
                ...
                // 与前一个角色的距离在 3 以上、且跟随者处于可见状态时执行路径探索
                this.setThrough(false);
    ...
    Game_Followers.prototype.initialize = function () {
        ...
        this._gatherCount = 0; // 用于监视 gather 超时
    };
    ...
    Game_Followers.prototype.areGathered = function () {
        // 经过 600 帧 gather 仍未结束时，为避免卡死而强制结束 gather
        if (this._gatherCount >= 600) {
            this._gatherCount = 0;
            return true;
        }
        // 由于 MV 中没有 Game_Follower#isGathered，因此重新定义 Game_Followers#areGathered 的处理

...
/*:
@target MV MZ
@plugindesc 像素级移动系统 决定触发器扩展 v1.1.1
@author unagi ootoro
...
使用本插件后，对于以决定键启动的事件，
可以设置其可启动范围。
还可以在玩家进入可启动范围时，让事件显示弹窗。
弹窗可以从气球图标、图标、图片、窗口中选用。

※ 导入本插件时，需要「DotMoveSystem.js v2.2.1」或更高版本。

【使用方法】
■ 弹窗显示的设置
在事件第0页的注释中记载以下内容，即可在事件变为可决定状态时
显示弹窗。
...
・显示消息窗口
<popupWindowText: 要显示的消息>
例：显示内容为 Test 的消息窗口时
<popupWindowText: test>

・指定窗口的字体大小
<popupWindowTextFontSize: 字体大小>
例：指定字体大小为 24 像素时
<popupWindowTextFontSize: 24>

・指定窗口的字体名称
<popupWindowTextFontFace: 字体名称>
例：指定 MS P Gothic 时
<popupWindowTextFontFace: MS PGothic>

・指定窗口皮肤图片
<popupWindowSkin: 图片文件名>
例：指定 Window_Popup.png 时
<popupWindowSkin: Window_Popup>

窗口皮肤图片必须放置在 system 文件夹中。
...
通过执行插件指令「弹窗参数更改」，
可以更改弹窗的设置。
参数的「设置内容」请按与在注释中设置时相同的格式书写。
※本功能仅限 MZ。MV 的情况下，可使用下方脚本重新设置。

■ 通过脚本更改弹窗显示时
通过使用脚本，可以在之后更改事件的弹窗显示。

・更改为显示图标
this.event().changePopupIcon(图标编号);
例：显示第 4 号图标时
this.event().changePopupIcon(4);

・更改为显示消息窗口
this.event().changePopupWindowText(要显示的消息);
例：显示内容为 Test 的消息窗口时
this.event().changePopupWindowText("Test");

・更改窗口的字体大小
this.event().changePopupWindowTextFontSize(字体大小);
例：更改为 32 像素时
this.event().changePopupWindowTextFontSize(32);

・更改窗口的字体名称
this.event().changePopupWindowTextFontFace(要显示的消息);
例：更改为 MS P Gothic 时
this.event().changePopupWindowTextFontFace("MSPGothic");

・更改为显示图片
this.event().changePopupImage(图片文件名);
例：显示 popup.png 时
this.event().changePopupImage("popup");

・更改为显示气球图标
this.event().changePopupBalloon(气球图标编号);
例：显示第 1 号气球图标时
this.event().changePopupBalloon(1);

・更改消息窗口的皮肤图片
this.event().changePopupWindowSkin(要显示的消息);
例：使用文件名为 "Window_MV" 的皮肤图片时
...
※注意
在通过脚本更改了设置的状态下，旧设置仍会残留。
因此请执行 clearPopupSetting 来清除旧设置。
另外，设置不会就这样直接反映到实际游戏画面上，
需要执行 applyPopupSetting，才能将设置更改反映到游戏画面。
...
@text 设置内容
@desc
指定要更改弹窗设置的事件ID。若指定 0，则以执行指令的目标事件为对象。
...
@default 2
@desc
指定可以执行决定触发的区域宽度。内部会将区域宽度作为圆的直径来处理。
...
@default 0
@desc
指定窗口中显示文本的字体大小。指定 0 则使用标准大小。

@param WindowTextFontFace
@text 窗口文本字体名称
@type string
@desc
指定窗口中显示文本的字体名称。留空则使用标准字体。

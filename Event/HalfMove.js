 ...
 * @param 8方向移動
 * @text 8方向移动
 * @desc 允许使用包括斜方向移动的8方向移动
 * @default true
 * @type boolean
 *
 * @param 8方向移動スイッチ
 * @text 8方向移动开关
 * @desc 只在指定开关ON时允许八方向移动。0则为总是允许。
 * @default 0
 * @type switch
 *
 * @param イベントすり抜け
 * @text 穿透事件
 * @desc 从侧面接触事件时，距离半步即可穿透
 * @default true
 * @type boolean
 *
 * @param 強制中無効
 ...
 * @param 角回避
 * @text 回避角落
 * @desc 直线移动中被地图角落卡住时，会切换为斜向移动并重试。但若行进方向上有可触发的事件，则该功能无效。
 * @default true
 * @type boolean
 *
 * @param 斜め移動中減速
 ...
 * @param トリガー拡大
 * @text 触发器扩大
 * @desc 扩展事件的触发区域。扩展区域依优先级而定。
 * @default false
 * @type boolean
 *
 * @param 実歩数調整
 * @text 真实步数调整
 * @desc 步数增加的时机是每2步增加1步。步数计数和伤害地形的时机也将被调整。
 * @default false
 * @type boolean
 *
 * @param 上半分移動不可地形
 * @text 上半部分不可通行地形
 * @desc 仅使图块上半部分变为不可通行的地形标签。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 上半分移動不可Region
 * @text 上半部分不可通行区域
 * @desc 仅使图块上半部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 下半分移動不可地形
 * @text 下半部分不可通行地形
 * @desc 仅使图块下半部分变为不可通行的地形标签。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 下半分移動不可Region
 * @text 下半部分不可通行区域
 * @desc 仅使图块下半部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右半分移動不可地形
 * @text 右半部分不可通行地形
 * @desc 仅使图块右半部分变为不可通行的地形标签。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右半分移動不可Region
 * @text 右半部分不可通行区域
 * @desc 仅使图块右半部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左半分移動不可地形
 * @text 左半部分不可通行地形
 * @desc 仅使图块左半部分变为不可通行的地形标签。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左半分移動不可Region
 * @text 左半部分不可通行区域
 * @desc 仅使图块左半部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右上移動不可地形
 * @text 右上部分不可通行区域
 * @desc 仅使图块右上部分变为不可通行的地形标志。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右上移動不可Region
 * @text 右上部分不可通行区域
 * @desc 仅使图块右上部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右下移動不可地形
 * @text 右下部分不可通行地形
 * @desc 仅使图块右下部分变为不可通行的地形标志。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 右下移動不可Region
 * @text 右下部分不可通行区域
 * @desc 仅使图块右下部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左上移動不可地形
 * @text 左上部分不可通行地形标志
 * @desc 仅使图块左上部分变为不可通行的地形标志。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左上移動不可Region
 * @text 左上部分不可通行区域
 * @desc 仅使图块左上部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左下移動不可地形
 * @text 左下部分不可通行地形标志
 * @desc 仅使图块左下部分变为不可通行的地形标志。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 左下移動不可Region
 * @text 左下部分不可通行区域
 * @desc 仅使图块左下部分变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 全方向移動不可地形
 * @text 全方向不可通行地形标志
 * @desc 仅使图块全方向变为不可通行的地形标志。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param 全方向移動不可Region
 * @text 全方向不可通行区域
 * @desc 仅使图块全方向变为不可通行的区域。若指定0，则无效。
 * @default ["0"]
 * @type number[]
 *
 * @param イベント複数起動防止
 * @text 防止事件同时触发
 * @desc 当同时存在多个事件满足触发条件时，仅启动ID最小的事件。
 * @default false
 * @type boolean
 *
 * @param イベント位置重複OK
 * @text 允许事件位置重复
 * @desc 优先级为“与角色相同”以外的事件，允许位置重复。
 * @default false
 * @type boolean
 *
 ...
 * 暂时禁止半步移动。玩家禁止半步移动后，可能无法启动半步位置的事件。
 * @text 目标
 * @desc 禁止半步移动的对象
 * @default -2
 * @type select
 ...
 * @min 1
 *
 * @help 角色的移动单位变为1个图块的一半。
 * 半步移动生效时，除载具外的所有对象都将变为半步移动。
 *
 * 要为每个事件单独使用扩展功能，请按如下方式在事件的备注栏中填写。
 * 
 *目标事件将不再半步移动。
 * <HM半歩禁止>
 * <HMHalfDisable>
 ...
 *・触发扩大设为ON时的规格
 * 可通过事件备注栏的记述单独设置触发区域。
 * // 想要向下、左、右、上方向分别扩大1格、2格、3格、4格时
 * <HM拡大領域:1,2,3,4>
 * 
 * 想要向下、左、右、上方向分别扩大0.5格、1格、1格、0.5格时
 * <HM拡大領域:0.5,1,1,0.5>
 * 
 *当优先级为“与普通角色相同”且事件穿透无效时，
 * 　左右方向各扩大半格的触发范围。
 * 
 *其他情况下，上下左右各扩大半格的触发范围。
 * 
 *将事件的初始位置加上半步行幅。
 * 请在想要从一开始就将事件放置在半步行位置时使用。
 * <HM初期半歩X:+>
 * <HMInitialHalfX:+>
 * <HM初期半歩Y:+>
 * <HMInitialHalfY:+>
 ...
 * 
 * ・事件的8方向自主移动
 *当玩家处于可8方向移动的状态时，事件在随机移动或
 * 接近玩家移动时也会使用8方向移动。
 * 通过指定以下标签，可以禁止事件的8方向移动。
 * <HM8MoveDisable>
 * <HM>
 * 
 * ・脚本（从移动路线设置的“脚本”中执行）
 *
 * 即使“强制移动路线期间半步行移动无效”的设置有效，也会以半步行进行强制移动。
 ...
 * 
 * 注意事项
 *将“强制中无效”参数设为ON并强制移动路线时，追随者的动作和通行判定
 * 会出现部分异常的问题，目前尚未解决。启用该参数时请多加注意。
 * 
 * 使用条款：
 *  可在未经作者许可的情况下进行修改、再分发，使用形式（商业用途、18禁用途等）
 * 也无任何限制。
 *  这个插件已经是属于你的了。
 */

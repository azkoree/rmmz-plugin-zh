...
/*:zh
 * @target MV MZ
 * @plugindesc v1.021 更改事件命令的目标
 * @author 砂川赳（http://newrpg.seesaa.net/）
 * @url http://newrpg.seesaa.net/article/482259070.html
 *
 * @help v1.021 更改事件命令的目标
 * 
 * 通过变量或数学公式，可以指定事件命令的对象。
 * 当对象为“本事件”时，会通过改写对象来实现处理。
 * 
 * 由此，可以对通常无法操作的追随者（队列行走的同伴）执行命令。
 * 
 * 例如，以下命令将成为对象：
 * 
 * - 移动路线的设置
 ...
 * ■ MV用插件指令
 * -------------------------------------------------------------------
 * ◆ NRP.EventCommandTarget.ChangeTarget [对象ID]
 * 更改作为“本事件”对象的角色。
 * 请使用数值或数学公式指定事件ID。
 * -1 为玩家，-2 以下为追随者。
 * 若留空则解除设置。
 ...
 * -------------------------------------------------------------------
 * 无特别限制。
 * 可自由修改、再分发，可商用，权利标识随意。
 * 作者不承担任何责任，但对于缺陷会在可能范围内予以应对。
 * @------------------------------------------------------------------
 * @ プラグインコマンド
 * @------------------------------------------------------------------
 * 
 ...
 * 留空为解除设定。
 * @text 目标ID
 * @desc 在调用的事件中以“本事件”为目标时，将指定ID的事件作为对象。
 * @value $gameVariables.value(1) #変数番のイベント
 * @type combo
 * @value -1 #プレイヤー
 * @option $gameVariables.value(1) #变量值对应事件
 * @value -2 #フォロワー
 * @option -1 #玩家
 * @value this._eventId + 1 #このイベント+1
 * @option -2 #随从
 * @value followerIdByActorId(1) #アクターＩＤのフォロワー
 * @option this._eventId + 1 #本事件 + 1
 * @value 0 #設定解除
 * @option followerIdByActorId(1) #对应角色ID的随从
 * @option 0 #解除设定
 * 
 * 

 ...
 * @param padCharacter
 * @text 填充字符
 * @desc 描画数值时，当位数不足指定位数时填充的字符。请指定半角1个字符。
 * @default 0
 *
 * @param prefixText
 * @text 前缀字符串
 * @desc 插入到所有字符串图片之前的文本。主要用于指定默认的控制字符等。
 * @default
 *
 * @param widthVariable
 * @text 宽度存储变量
 * @desc 存储最后绘制的动态字符串宽度（像素数）的变量。
 * @default 0
 * @type variable
 *
 * @param heightVariable
 * @text 高度存储变量
 * @desc 存储最后绘制的动态字符串高度（像素数）的变量。
 * @default 0
 * @type variable
 *
 * @param betweenVariableId
 * @text 字间距变量编号
 * @desc 获取字间距值（像素单位）的变量编号。需要另外的字间距设置插件。
 * @default 0
 * @type variable
 *
 * @command dText
 ...
 * @arg fontSize
 * @text 字体大小
 * @desc 动态字符串的初始字体大小。指定0则使用系统设置的默认大小。
 * @default 0
 * @type number
 *
 * @command dTextSetting
 * @text 字符串图片设置
 * @desc 关于字符串图片显示方法的设置。更改后的设置在绘制后也会保持。未设置的项目保持现状。
 *
 * @arg backGroundColor
 * @text 背景色
 * @desc 字符串图片的背景色。
 ...
 * @arg gradationLeft
 * @text 左侧渐变
 * @desc 字符串图片背景左侧渐变的像素数。
 * @type number
 * @default
 *
 * @arg gradationRight
 * @text 右侧渐变
 * @desc 字符串图片背景右侧渐变的像素数。
 * @type number
 * @default
 *
 * @arg realTime
 * @text 实时绘制
 * @desc 使用控制字符\v[n]时，如果变量内容发生变化则重新绘制图片。
 * @type boolean
 * @default
 *
 * @arg window
 ...
 * @arg pictureWidth
 * @text 宽度
 * @desc 字符串图片的宽度。在与文字绘制宽度无关的情况下指定尺寸时使用。
 * @type number
 * @default
 *
 * @arg pictureHeight
 * @text 高度
 * @desc 字符串图片的高度。在与文字绘制高度无关的情况下指定尺寸时使用。
 * @type number
 * @default
 *
 * @arg align
 ...
 * @arg fontFace
 * @text 字体
 * @desc 字符串图片的字体。指定通过字体加载插件加载的字体名称。
 * @default
 *
 * @arg lineHeight
 * @text 行高
 ...
 * @arg vertical
 * @text 简易竖排
 * @desc 将字符串图片设为竖排。这是简易功能。无法竖排2行以上的文章。
 * @default false
 * @type boolean
 *
 * @command dTextSettingClear
 * @text 字符串图片设置解除
 * @desc 解除并初始化所有关于字符串图片显示方法的设置。
 *
 * @command windowCursor
 * @text 窗口光标设置
 * @desc 在显示中的字符串图片背景窗口显示光标。
 *
 * @arg pictureId
 * @text 图片编号
 * @desc 目标图片编号。必须是已经显示中的图片。
 * @type number
 * @default 1
 *
 * @arg x
 ...
 * @arg activateSwitch
 * @text 有效开关
 * @desc 指定后，仅在开关为ON时激活。未指定时始终激活。
 * @type switch
 * @default 0
 * 
 * @help 提供指令，能够动态生成字符串图片。
 * 字符串可以使用各种\v[n]之类的控制符，通过控制符显示的变量值变化时，能够
 * 实时更新字符串图片的内容。
 *
 * 按以下步骤显示：
 *  1 : 使用插件命令[字符串图片准备]准备绘制内容
 *  2 : 使用事件命令「显示图片」将「图像」指定为未选择。
 * ※ 在步骤1时图片不会显示，所以请务必组合调用。
 *
 * 专用控制字符
 * \V[n,m](用参数指定的字符填充m位数的变量值)
 * \item[n]   n 号物品信息（图标＋名称）
 * \weapon[n] n 号武器信息（图标＋名称）
 * \armor[n]  n 号防具信息（图标＋名称）
 * \skill[n]  n 号技能信息（图标＋名称）
 * \state[n]  n 号状态信息（图标＋名称）
 * \oc[c] 将轮廓颜色设置为「c」(※1)
 * \ow[n] 将轮廓宽度设置为「n」(例:\ow[5])
 * \f[b] 字体加粗
 * \f[i] 字体斜体
 * \f[n] 将字体加粗和斜体恢复为正常
 *
 * ※1 轮廓颜色的指定方法
 * \oc[red]  用颜色名指定
 * \oc[rgb(0,255,0)] 用颜色代码指定
 * \oc[2] 用与文字颜色编号\c[n]相同的方式指定
 *
 * 如果想更改动态字符串图片的字体，
 * 请另外使用字体加载插件来加载。
 ...
 * 『PluginCommonBase.js』存储在RPG Maker MZ安装文件夹下的
 * 以下文件夹中。
 * dlc/BasicResources/plugins/official
 *
 * 使用条款：
 *  可以在未经作者许可的情况下修改、再分发，对使用形式（商用、18禁使用等）
 *  也没有限制。
 *  这个插件已经是你的了。
 */
/*:

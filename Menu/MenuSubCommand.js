 ...
 * @text 命令添加位置
 * @desc 添加子命令群的位置。指定0时，会添加到窗口的开头。
 * @default 0
 * @type number
 *
 * @param subMenuWidth
 * @text 子菜单宽度
 * @desc 显示子菜单的窗口宽度。不指定时将应用默认值「240」。
 * @default 0
 * @type number
 *
 * @param selectActorIdVariable
 * @text 选择角色ID变量
 * @desc 从子命令选择角色时，存储该角色ID的变量编号。
 * @default 0
 * @type variable
 *
 * @param windowSkin
 ...
 * @param subMenuX
 * @text 子菜单X坐标
 * @desc 指定后子命令的X坐标将变为固定值。
 * @default 0
 * @type number
 *
 * @param subMenuY
 * @text 子菜单Y坐标
 * @desc 指定后子命令的Y坐标将变为固定值。
 * @default 0
 * @type number
 *
 * @param adjustX
 ...
 * @text 菜单图片单独管理
 * @desc 分别管理菜单地图和普通地图的图片显示状态。
 * @default false
 * @type boolean
 *
 * @param autoTransparent
 * @text 自动透明化
 * @desc 移动到子地图时自动将玩家设为透明。
 * @default true
 * @type boolean
 *
 * @help MenuSubCommand.js
 *
 * 可以在主菜单画面中添加任意名称的命令以及树形显示的子命令，数量不限。
 * 执行（确定）子命令时，可以执行任意脚本，
 * 或者移动到指定的地图。（两者也可以同时进行）
 *
 * 脚本主要用于通过脚本编写的其他画面。
 * 当然也可以跳转到其他插件添加的画面。
 * 地图移动主要用于通过事件跳转到自制的菜单画面。
 * 从自制菜单画面返回时，请再次打开菜单。
 * 原本打开菜单的位置已经单独保存，无需在意。
 *
 * 此外，除了通常的纵向布局菜单画面，也支持插件制作的横向布局菜单画面。
 *
 * 如果所有子命令都为非显示状态，父项本身也会变为非显示状态。
 * 同样，如果所有子命令都为禁止使用状态，父项本身也会变为禁止使用状态。
 *
 * 如果子命令只有一个，子命令窗口将不会显示，
 * 在选择父命令时就会执行子命令。
 *
 * 此插件没有插件命令。
 *
 * 使用条款：
 *  可以在未经作者许可的情况下进行修改、再分发，使用形态（商用、18禁使用等）
 *  也没有限制。
 *  这个插件已经是你的了。
 */

/*~struct~SubCommand:zh
 *
 * @param CommandId
 * @text 命令ID
 * @desc 子命令的识别编号。通过这个编号和名称将命令汇总。通常全部设为0即可。
 * @default 0
 *
 * @param Name
 * @text 命令名称
 ...
 * @param ParentName
 * @text 父名称
 * @desc 在主命令中显示的子命令的父名称。具有相同父名称的子命令会被汇总为一个。
 * @default 父命令1
 *
 * @param HiddenSwitchId
 * @text 非显示开关编号
 * @desc ON时命令变为非显示状态的开关ID
 * @default 0
 * @type switch
 *
 * @param DisableSwitchId
 * @text 使用禁止开关编号
 * @desc ON时命令变为禁止使用状态的开关ID
 * @default 0
 * @type switch
 *
 * @param Script
 ...
 * @default
 * @type combo
 * @option this.commandItem(); // 打开物品画面
 * @option this.commandSave(); // 打开存档画面
 * @option SceneManager.push(Scene_Load); // 打开读档画面
 * @option this.commandOptions(); // 打开选项画面
 * @option this.commandGlossary(1); // 调用语料词典(使用语料词典插件时)
 * @option $gameSwitches.setValue(1, true); // 将开关[1]设为ON
 * @option $gameVariables.setValue(1, 10); // 将变量[1]设为[10]
 * @option $gameTemp.reserveCommonEvent(1); // 返回地图后调用公共事件[1]
 *
 * @param ReturnMap
 * @text 是否返回地图
 * @desc 脚本执行后返回地图。

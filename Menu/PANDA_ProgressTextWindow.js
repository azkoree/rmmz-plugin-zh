...
/*:zh
 * @target MV MZ
 * @plugindesc 在菜单显示一个文本框，用于显示当前游戏进度
 * @author panda(werepanda.jp)
 * @url http://www.werepanda.jp/blog/20210125012307.html
 * 
 * @help
 * 在菜单画面添加一个显示文本的窗口。
 * 根据插件参数中设置的变量值，显示对应的文本。
 * 可用于显示故事的进度，或者显示下一个目的地之类的。
 * 
 * 可以通过调整XY坐标和宽高度，自由配置窗口的位置。
 * 默认状态下，会显示在菜单界面的空位。
 * MZ时，在菜单画面顶部、取消按钮左侧显示。
 * MV时，在菜单命令和金钱窗口之间显示。
 * 
 * ■ 利用规约
 * 本插件以 MIT 许可证发布。
 ...
 * @param ProgressText
 * @text 进度文本
 * @desc 根据进度变量的值显示的文本列表。进度从1开始。"\n"换行。
 * @type string[]
 * @default 
 * 
 * @param TextWidthAutoAdjust
 * @text 宽度适应文本
 * @desc 启用后窗口会根据文本长度自动调整宽度。但是无法使用换行控制符。
 * @type boolean
 * @default false
 * 
 * @param WindowLayout
 * @text 窗口布局
 * @desc 设置窗口的位置和尺寸。默认情况下在菜单画面的空位。
 * @type struct<rect>
 * 
 */
/*~struct~rect:zh:
...
(() => {
	...
	Window_Progress.prototype.setProgressText = function() {
		...
		if (p === 60 && $gameSwitches.value(169)) {	// ad-hoc
			const text = ProgressText[199] || '';
			this.setText(text);
		} else {

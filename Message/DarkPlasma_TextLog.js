 ...
 * @desc 当设定的开关为ON时不会记录日志。为0时，始终记录日志。
 * @text 日志记录无效开关
 * @type switch
 * @default 0
 *
 * @param openLogKeys
 * @desc 设置用于打开/关闭文本日志窗口的按钮。
 * @text 日志开闭按钮
 * @type select[]
 * @option shift
 * @option control
 ...
 * @desc 当设定的开关为ON时无法打开日志窗口。为0时，始终可以打开。
 * @text 日志窗口无效开关
 * @type switch
 * @default 0
 *
 ...
 * @desc 设置日志中消息的间隔。消息以事件命令为单位作为一个整体。
 * @text 消息间隔
 * @type number
 * @default 0
 *
 * @param logSplitter
 * @desc 设置用于插入事件结束等位置的分割线。
 * @text 日志分割线
 * @type string
 * @default -------------------------------------------------------
 *
 * @param autoSplit
 * @desc 为ON时，除战斗、公共、并行事件外，事件结束时自动插入分割线。
 * @text 自动分割线
 * @type boolean
 * @default true
 *
 * @param choiceFormat
 * @desc 设置日志中显示的选项格式。{choice}会被替换为选择的选项。
 * @text 选项格式
 * @type string
 * @default 選択肢:{choice}
 *
 ...
 * @desc 如果要在日志窗口中忽略逐次处理的控制字符\XXX，请在此添加XXX。
 * @text 忽略的控制字符
 * @type string[]
 * @default []
 *
 * @param scrollSpeed
 * @desc 设置上下键滚动的速度。数值越大速度越快。
 * @text 滚动速度
 * @type number
 * @default 1
 * @min 1
 ...
 * @desc 设置 PageUp/PageDown 键滚动的速度。
 * @text 高速滚动速度
 * @type number
 * @default 10
 * @min 1
 ...
 * @desc 设置保存的日志消息数量。请注意，设置过多可能会影响游戏运行。
 * @text 日志消息保存数
 * @type number
 * @default 200
 *
 ...
 * 日志不会保存在存档数据中。
 *
 * 在地图上、事件中按下日志开闭键可以打开日志窗口。
 * 使用日志开闭键、确定键或取消键中的任意一个可以关闭日志窗口。
 *
 * 关于忽略的控制文字设置
 * 只能忽略消息显示时逐次处理的控制文字。
 * 无法忽略 \V \S 等在消息显示处理开始时
 * 进行转换处理的控制文字。
 *
 * 通过以下脚本，可以从地图打开日志。

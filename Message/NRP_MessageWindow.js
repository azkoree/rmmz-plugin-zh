 ...
 * 可以使文章更容易阅读。
 * 
 * 调查一下就会发现，现在的JRPG和ADV
 * 2～3行+名字栏的样式是主流。
 * MZ默认的4行+名字栏的样式，
 * 文章量相当多，字体也容易变小，所以很难阅读。
 * 不推荐不加思考地沿用初始状态。
 * 
 * 此时，将「一行的高度」加宽是关键。
 * 特别是与给汉字添加注音的插件并用时，
 ...
 * -------------------------------------------------------------------
 * 大部分项目都可以使用公式。
 * 例如，将「窗口的高度」设置为「$gameVariables.value(1)」，
 * 消息窗口的高度就会根据变量1的值而变化。
 * 
 * 例如，「在显示立绘的场景中想要将文章设为2行」
 * 这种情况下很方便。
 * 
 * -------------------------------------------------------------------
 * ■窗口的图像化
 * -------------------------------------------------------------------
 * 可以用图像显示消息窗口和名字栏。
 * 请将图像放置在img\system文件夹中。
 * 
 * 支持两种方法：一种是消息窗口和名字栏使用不同的图像，
 * 另一种是只在显示名字时使用不同的图像。
 * 
 * 另外，因为这样会保留原来的窗口，
 * 所以请将不透明度设为0，并隐藏边框。
 * ※也可以在保留原窗口的基础上用图像进行装饰。
 * 
 * -------------------------------------------------------------------
 * ■遮罩图像功能
 * -------------------------------------------------------------------
 * 通过设置「遮罩图像」，可以细致地设置窗口的透明度。
 * 例如，可以只提高窗口右侧的透明度。
 * 遮罩图像的白色部分是不透明的，黑色部分是透明的。
 * ※详情请参考示例图像。
 * 
 * 另外，由于某种原因在RPG Maker MV中无法使用，所以是MZ专用。
 * 
 * -------------------------------------------------------------------
 * ■使用条款
 * -------------------------------------------------------------------
 * 没有特别限制。
 * 可以自由修改、再分发、商用使用，权利表示也是可选的。
 * 作者不承担责任，但会在可能的范围内处理故障。
 * 
 * @------------------------------------------------------------------
 * @ プラグインパラメータ
 * @------------------------------------------------------------------
 ...
 * @desc 显示消息窗口的Y坐标。
 * 初始值：(this._positionType * (Graphics.boxHeight - this.height)) / 2
 * 
 * @param LineHeight
 * @parent <MessageWindow>
 * @text 一行高度
 ...
 * @type file
 * @dir img/system
 * @desc 对窗口进行半透明处理的遮罩图像。
 * 此功能为MZ专用。
 * 
 * @param NoMaskOpacity
 * @parent MaskImage
 * @text 非遮罩对象的不透明度
 * @desc 无法对消息窗口应用遮罩时的不透明度。
 * 
 * @param FixIconY
 * @parent <MessageWindow>
 * @text 图标Y坐标修正
 * @type boolean
 * @default true
 * @desc 修改一行高度时，修正图标Y坐标偏移的问题。
 * 
 * @param <NameBoxWindow>
 * @text ＜名字栏＞
 * @desc 与名字栏相关的项目。
 * 在RPG Maker MV中不存在，所以无效。
 * 

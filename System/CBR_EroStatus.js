...
/*:zh
 * @target MZ
 * @plugindesc 增加一个可显示各种状态的界面
 * @author 
 *
 * @param SE
 * @text SE
 * @default Cursor3
 * @desc 指定切换页面时播放的音效，默认：Cursor3
 *
 * @param SEvol
 * @text SE音量
 * @type number
 ...
 * 
 * ~设定~
 * 在脚本第一行输入“CBR-エロステータス”，标志此处使用该插件功能
 * 
 * ~初始化~
 * “初期化-???” 问号处写ALL将初始化所有页面，写数值仅初始化对应页面
 * 
 * ~输入数据~
 * “ページ-???”问号处输入要设置数据的界面
 * 
 ...
 * ※注※ 务必将“画像-????.png”放在第二行
 * 
 * “画像-??????.png”显示在pictures文件夹保存的图像，可以使用\V[n]控制符
 * “x-???” 指定x坐标
 * “y-???” y坐标
 * “透明度-???” 数值范围0~100 可选，默认为100
 * “サイズ-???” 图像尺寸 数值范围0~ 可选，默认为100
 * 
 * ~文本~
 * ※注※ 务必将“テキスト-???”放在第二行
 * 
 * “テキスト-??????” 显示文本 支持\v[n]和\{控制符
 * “x-???” 指定x坐标
 * “y-???” y坐标
 * “左右-?” 将x坐标原点指定为左、中、右 可选，默认为左
 * “上下-?” 将文本的上下对齐指定为上、中、下 可选，默认为上
 * “サイズ-???” 文本尺寸 可选，默认为28
 * 
 *
 * 

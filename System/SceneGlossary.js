 ...
 * @param GlossaryInfo
 * @text 用语信息(必须设置)
 * @desc 用语辞典信息。可以添加任意用语辞典。必须至少注册1个用语。
 ...
 * @param Layout
 * @text 布局
 * @desc 布局设置相关项目。这是汇总用的项目，请勿在此输入任何内容。
 *
 * @param FontSize
 * @text 字体大小
 * @desc 用语集的字体大小。
 ...
 * @text 图片自动缩小
 * @desc 在窗口内显示图片时，会自动缩小以适应显示。如果指定了文本位置则无效。
 * @default true
 * @type boolean
 * @parent Layout
 *
 ...
 * @text 图片显示位置
 * @desc 图片的显示位置。(top:窗口顶部 bottom:窗口底部 text:文本末尾)
 * @default top
 * @type select
 * @option top
 * @option bottom
 ...
 * @param TextPosition
 * @text 文本显示位置
 * @desc 文本的显示Y坐标。为0时根据图片显示位置自动设置。
 * @default 0
 * @type number
 *
 * @param PictureAlign
 * @text 图片对齐
 * @desc 图片的对齐方式。(left:左对齐 center:居中对齐 right:右对齐)
 * @default center
 * @type select
 * @option left
 * @option center
 ...
 * @text 图片优先度
 * @desc 图片的显示优先级。(top:文本上方 bottom:文本下方)
 * @default top
 * @type select
 * @option top
 * @option bottom
 ...
 * @param AutoAddition
 * @text 自动注册
 * @desc 在显示文本的命令中出现相同单词时自动注册。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @param AutoAdditionEnemy
 * @text 敌人自动注册
 * @desc 击败敌人时自动注册与敌人同名的单词。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @param SwitchAutoAdd
 * @text 自动注册ID开关
 * @desc 用语项目自动注册时，指定编号的开关会变为ON。需要进行某种通知时指定。
 * @default 0
 * @type switch
 * @parent AutoAddition
 *
 ...
 * @text 自动注册ID变量
 * @desc 用语项目自动注册时，将项目ID设置到指定编号的变量中。
 * @default 0
 * @type variable
 * @parent AutoAddition
 *
 * @param CategoryOrder
 * @text 分类排列顺序
 * @desc 如果要任意更改分类排列顺序，请指定分类名称。
 * @default
 * @type string[]
 *
 * @param CategoryUnusable
 * @text 禁用分类
 * @desc 此处指定的分类即使「项目使用」有效也无法使用。
 * @default
 * @type string[]
 *
 * @param ShowPageNumber
 * @text 页码显示
 * @desc 对于多页用语，在用语辞典底部显示页码。
 * @default true
 * @type boolean
 *
 * @noteParam SGピクチャ
 ...
 * @arg category
 * @text 分类
 * @desc 指定时，用语辞典将在选中分类的状态下打开。
 * @default
 *
 * @arg listIndex
 * @text 列表编号
 * @desc 指定时，用语辞典将在选中列表编号的状态下打开。指定时必须指定分类。
 * @default 0
 * @type number
 *
 * @command GLOSSARY_GAIN_ALL
 * @text 获取全部用语
 * @desc 将数据库中注册的所有用语设为已获取状态。
 *
 * @command GLOSSARY_LOSE_ALL
 * @text 丢弃全部用语
 * @desc 数据库中注册的所有用语项目将丢失。
 *
 * @command GLOSSARY_BACK
 * @text 返回用语集画面
 * @desc 在重新选中最后选择项目的状态下调用用语集画面。
 *
 * @command GLOSSARY_ITEM_CHANGE_CATEGORY
 * @text 更改用语项目的分类
 * @desc 将指定ID项目的分类更改为其他分类。只能更改项目。无法更改武器和防具。
 *
 * @arg itemId
 * @text 项目ID
 * @desc 分类更改对象的项目ID。
 ...
 * ・使用方法
 * 1. 请从数据库注册作为用语的项目。
 * 详细的注册方法，请参考后述的「数据注册方法」。
 *
 * 2. 请设置参数的「用语信息」。（双击项目）
 * 注意：如果不设置该参数而从插件命令启动用语辞典会报错。
 *
 * 用语除了通过获取对象项目来浏览外，还有在显示文本的命令中
 * 出现相同单词时自动注册的功能。
 * （也可以指定特定用语不作为自动注册对象）
 *
 * 用语可以选择在一个窗口中显示所有的方式
 * 和按分类分类显示的方式。
 * 请从参数选择显示方法。
 * 按分类显示时，请在备注栏指定「<SGCategory:xxx>」。
 * 用逗号分隔指定分类，可以属于多个分类。
 *
 * 例：<SGCategory:xxx, yyy>
 *
 * 可以从菜单画面和插件命令转移到用语集画面。
 *
 * ・数据注册方法
 * 1.在项目数据库中注册新数据
 * 将「项目类型」设置为「隐藏项目A」或「隐藏项目B」
 *
 * 2.在「名称」中设置用语的名称
 *
 * 3.在「备注栏」中按以下方式记述（不需要的项目可以省略）
 * <SGDescription:xxx>           // 用语的说明文(※1)
 * <SG共通説明:説明文>       // 用语的共用说明文(重复使用用)
 * <SG未入手説明:説明文>      // 未入手用语的说明文
 * <SGCategory:xxx>   // 用语所属的分类名称
 * <SGManual>                  // 将用语从自动注册对象中排除
 * <SGPicture:filename>   // 用语的图片文件名
 * <SGEnemy:1>   // 显示敌人图片代替图片(※2)
 * <SGPicturePosition:text>     // 图片的显示位置
 * top, bottom, text
 * <SGピクチャX:100>        // 用语的图片X坐标修正
 * <SGピクチャY:100>        // 用语的图片Y坐标修正
 * <SGTextPosition:100>     // 文本的显示位置
 *  top:窗口顶部 bottom:窗口底部 text:文本末尾
 * <SGPicturePriority:top>   // 图片的显示优先级
 *  top:文本上方 bottom:文本下方
 * <SGPictureScale:0.5>    // 图片的放大率
 * <SGPictureAlign:right>     // 图片的对齐
 *  left:左对齐 center:居中对齐 right:右对齐
 * <SGNoCollect>             // 将用语设置为收集率计算对象外
 * <SGTextColorChange:1,10>   // 开关[1]为ON时将文字色更改为[10]
 *
 * ※1 可以使用以下特殊的控制字符。
 * \COMMON[1]  // 替换为ID[1]的项目的<SG共通説明:aaa>。
 * \mhp[3]     // 对象敌人的最大HP(3位零填充)
 * \mmp[3]     // 对象敌人的最大MP(3位零填充)
 * \atk[3]     // 对象敌人的攻击力(3位零填充)
 * \def[3]     // 对象敌人的防御力(3位零填充)
 * \mag[3]     // 对象敌人的魔法力(3位零填充)
 * \mdf[3]     // 对象敌人的魔法防御(3位零填充)
 * \agi[3]     // 对象敌人的敏捷性(3位零填充)
 * \luk[3]     // 对象敌人的运气(3位零填充)
 * \exp[3]     // 对象敌人的获得经验值(3位零填充)
 * \money[3]   // 对象敌人的获得金币(3位零填充)
 * \drop[1]    // 对象敌人的[1]号掉落项目
 *
 * ※2 省略敌人ID时，会自动设置与用语项目同名的敌人。
 *
 * \DATA[prop] // 替换为对象数据的属性「prop」。(下述参照)
 * \DATA[description] // 对象数据的说明
 * \DATA[price]       // 对象数据的价格
 * \EFFECT_TYPE[0] // 对象数据的[0]号使用效果的种类
 * \SCRIPT{aaa}  // 脚本[aaa]的评估结果。
 *               // 可以用局部变量[data]参照对象数据。
 *
 * ・使用例1(0号使用效果的效果量(%))
 ...
 * 此外，一个用语可以使用多个页面。
 * 用方向键左右切换页面。
 * <SGDescription2:xxx>          // 第2页的用语说明文
 * <SGPicture2:filename>  // 第2页的用语图片文件名
 * <SGPicturePosition2:text>    // 第2页的图片显示位置
 * <SG表示スイッチ2:1>       // 仅在开关[1]为ON时显示第2页以后(※)
 * ※如果隐藏第2页，第3页以后也会自动隐藏。
 *
 * 第3页以后也同样，最多可以指定99页。
 * 多页显示时的第1页请不要加「1」。
 ...
 *
 * 要在不同种类的用语辞典中显示用语，需要以下标签。
 * <SGType:2>   // 用语所属的种类编号
 *
 * 要与「YEP_MainMenuManager.js」协作，进行命令的显示控制，
 * 请在将「命令名称」项目设为空的基础上，将「YEP_MainMenuManager.js」的
 * 参数设置如下。
 *
 * Menu X Name      : '用語辞典1'
 ...
 *
 * ・追加功能1
 * 非隐藏项目的「项目」「武器」「防具」也可以在辞典画面
 * 显示。在备注栏中填写与隐藏项目相同的内容。
 * 也可以制作项目图鉴、武器图鉴、防具图鉴。
 * 使用此功能时，请启用参数「使用入手历史」。
 *
 * ・追加功能2
 ...
 * 可以用以下标签单独设置显示顺序。
 * 同一显示顺序重复时，ID较小的先显示。
 * <SG表示順:5> // 与ID[5]相同的排列顺序显示。
 * <SGOrder:5>  // 同上
 *
 * ・追加功能3
 * 支持制作怪物辞典。
 * 1. 用参数「敌人自动注册」获取与战斗过的敌人同名的用语
 * 2. 用备注栏<SG敵キャラ>显示对象敌人的图片
 * 3. 用\mhp[3]等控制字符显示对象敌人的参数
 *
 * ・追加功能4
 * 可以对一个用语显示多个图片。
 * 在备注栏中记述以下标签，用逗号分隔文件名、X坐标、Y坐标
 * 指定。
 * <SG追加1ピクチャ:aaa,1,2> // 在X[1] Y[2]显示图片「aaa」
 *
 * 如果想进一步添加显示，请如下记述。
 * <SG追加2ピクチャ:bbb,2,3>
 *
 * ・追加功能5
 * 可以给用语辞典添加页码。请从参数启用设置。
 * 如果不想按用语单位显示页码，可以用以下备注栏对应。
 * <SGページ番号不要>
 * <SGNoPageNum>
 *
 * ・追加功能6
 ...
 *
 * ・脚本详细
 * 当itemId可以作为用语项目使用时返回true。
 * $gameParty.isUsableGlossaryItem(itemId);
 *
 * 返回与指定的分类名和用语种类名对应的收集率。
 * 省略用语种类时设置为[1]。
...
(function() {
    ...
    Window_GlossaryCategory.prototype.initialize = function(glWindow) {
        ...
        // 创建合适的矩形区域而不是直接传递窗口对象
        const rect = new Rectangle(glWindow.x, glWindow.y, glWindow.width, glWindow.height);
    ...
    Window_GlossaryCategory.prototype.makeItemList = function() {
        ...
        // 过滤掉可能的无效数据
        this._data = this._data.filter(item => item !== undefined && item !== null);
    };

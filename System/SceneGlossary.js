 ...
 * <SGカテゴリ:カテゴリ名>   // 用语所属的分类名称
 * <SG手動>                  // 将用语从自动注册对象中排除
 * <SGピクチャ:ファイル名>   // 用语的图片文件名
 * <SG敵キャラ:敵キャラID>   // 显示敌人图片代替图片(※2)
 * <SGピクチャ位置:text>     // 图片的显示位置
 * <SGピクチャX:100>        // 用语的图片X坐标修正
 * <SGピクチャY:100>        // 用语的图片Y坐标修正
 * <SGテキスト位置:100>      // 文本的显示位置
 *  top:窗口顶部 bottom:窗口底部 text:文本末尾
 * <SGピクチャ優先度:top>    // 图片的显示优先级
 *  top:文本上方 bottom:文本下方
 * <SGピクチャ拡大率:0.5>    // 图片的放大率
 * <SGピクチャ揃え:right>    // 图片的对齐
 *  left:左对齐 center:居中对齐 right:右对齐
 * <SG収集対象外>            // 将用语设置为收集率计算对象外
 * <SGテキスト色変化:1,10>   // 开关[1]为ON时将文字色更改为[10]
 *
 * ※1 可以使用以下特殊的控制字符。
 ...
 * 用方向键左右切换页面。
 * <SG説明2:説明文>          // 第2页的用语说明文
 * <SGピクチャ2:ファイル名>  // 第2页的用语图片文件名
 * <SGピクチャ位置2:text>    // 第2页的图片显示位置
